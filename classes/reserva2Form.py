...
class Reserva2Form(Gclass):
    ...
    def __init__(self, code,classEvents, selectedDiaHora):
        ...
        if code == 'None':
            codes = Reserva2Form.getatlist('_code')
            if codes == []:
                code = str(1)
            else:
                code = str(max(map(int,Reserva2Form.getatlist('_code'))) + 1)
        # Object attributes
        self.code = code
        self.classEvents = classEvents
        

        self.selectedDiaHora = Reserva2Form.str2datetimeDiaHora(selectedDiaHora)
        ...
        Reserva2Form.obj[code] = self
        Reserva2Form.lst.append(code)
#%% property
    # Object properties
    # getter methodes
...
#%% def    
    ...
    def text_to_horario(self,evento):
        texto = f"{evento.codigo} - {evento.checkout} - {evento.codigo_cliente} "
